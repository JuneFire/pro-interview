package com.leetcode.huawei.C204最长合法表达式;
/*
提取字符串中的最长合法简单数学表达式字符串长度最长的，并计算表达式的值。如果没有返回0。
        简单数学表达式只能包含以下内容
        0-9 数字，符号+-*
        说明：
...
public class Main {
    public static void main(String[] args) throws ScriptException {
        ...
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            if (Character.isDigit(c) || (c == '+' || c == '-' || c == '*') && (i == 0 || Character.isDigit(s.charAt(i - 1)))) {
        ...
        long result = ((Number) engine.eval(longestExpression)).longValue();
